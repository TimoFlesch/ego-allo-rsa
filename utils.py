'''
Author: Xuan Wen
Date: 2021-02-22 17:11:01
LastEditTime: 2021-03-30 16:49:05
FilePath: /rnn_sc_wc/create_input.py
'''
import math
import numpy as np
...
def fit_transform(data, label_type = None):
    ...
    if label_type == 'Polar':
        ...
        scaler = [abs(np.max(data[:,0]) - np.min(data[:,0])), np.max(data[:,1]) - np.min(data[:,1])]
    else:
        return None
...
def random_poke_generator(num_element=5):
    # fix random seed to fix output numbers
    return random.randint(1, 49, size=(2, num_element))

def front_frame(random_seed = 20 ,frame_amount = 20):
    random.seed(random_seed)
    front_frame_size = 50
    poke_size = 2
    frames = np.zeros((frame_amount, front_frame_size, front_frame_size))
    frames[:][:][:] = 0.1
    start_poke_coordinate = random_poke_generator(frame_amount)
    target_poke_coordinate = random_poke_generator(frame_amount)

    for num in range(0, frame_amount):
        empty_poke_coordinate = random_poke_generator()
        # add noise pokes
        for index in range(0, len(empty_poke_coordinate[0])):
            for poke_row in range(poke_size):
                for poke_column in range(poke_size):
                    frames[num][empty_poke_coordinate[0][index]+poke_row][empty_poke_coordinate[1][index]+poke_column] = 0.3
        # add start poke
        for poke_row in range(poke_size):
            for poke_column in range(poke_size):
                frames[num][start_poke_coordinate[0][num]+poke_row][start_poke_coordinate[1][num]+poke_column] = 0.7
        # add target poke
        for poke_row in range(poke_size):
            for poke_column in range(poke_size):
                frames[num][target_poke_coordinate[0][num]+poke_row][target_poke_coordinate[1][num]+poke_column] = 1
...
def circum(r = 10):
    unsorted = np.unique(np.asarray([np.asarray([math.floor(float(i)) for i in x])+r for x in float_points_in_circum(r)]), axis=0)
...
def input_frame(front_frame, output_type, start_poke=None):
    ...
    """
    background_size = 100
    conbined_frame = np.zeros(
        (len(front_frame), background_size, background_size))
    if output_type == "WC":
        for index in range(0, len(front_frame)):
            conbined_frame[index, 24:24+50, 24:24+50] = front_frame[index]
    elif output_type == "SC" and start_poke.all() != None:
        for index in range(0, len(front_frame)):
            #conbined_frame[a, b, c]
            conbined_frame[index, 50-start_poke[0][index]:100-start_poke[0][index],
                           50-start_poke[1][index]:100-start_poke[1][index]] = front_frame[index]
    else:
        return 0
    ...
    for index in range(0, len(start_poke[0])):
        if output_type == "WC":
            ...
            if label_type == "Cartesian":
                label[index] = world_center_distance_cartesian(target = [ target_poke[0][index], target_poke[1][index] ])
            elif label_type == "Polar":
                label[index] = world_center_distance_polar(target = [ target_poke[0][index], target_poke[1][index] ])
            else:
                return 0
            ...
            if label_type == "Cartesian":
                label[index] = self_center_distance_cartesian(start = [ start_poke[0][index], start_poke[1][index] ] ,target = [ target_poke[0][index], target_poke[1][index] ])
            elif label_type == "Polar":
                label[index] = self_center_distance_polar(start = [ start_poke[0][index], start_poke[1][index] ] ,target = [ target_poke[0][index], target_poke[1][index] ])
            else:
                return 0
...
def world_center_distance_polar(target):
    ...
    # #     return [math.pi, math.sqrt(math.pow(vector[0], 2) + math.pow(vector[1], 2))]
    # else:
    return [math.atan2(vector[1],vector[0]), math.sqrt(math.pow(vector[0], 2) + math.pow(vector[1], 2))]
...
def RSA_random_input_generate():
    ...
    unique_x = [*map(lambda ele: next(tup for tup in sort_by_x if tup[1][0] == ele),
        {tup[1][0] for tup in sort_by_x})]
    
    unique_y = [*map(lambda ele: next(tup for tup in sort_by_y if tup[1][1] == ele),
...
def example_gif_generate():
    ...
    for i in range(len(frames)):
    	new_frames[i] = cv2.resize(frames[i], dsize=(500, 500), interpolation=cv2.INTER_NEAREST)
